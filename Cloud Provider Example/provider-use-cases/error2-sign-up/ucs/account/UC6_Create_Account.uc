...
Variation: 1a User signs up using external service.
1a1 User authenicates himself using external authentication 
    service. 
1a2 System validates that user does not have account in the
    system yet.
1a3 System notes that user has been validated through
    external service #(mark:external_auth) #(goto:6)

Extension: 1a2a Account already registered.
1a2a1 System detects already existing account. 
      #(guard:account_created)
1a2a2 System shows message that account is already 
      registered. #(abort)

Variation: 8a User authenticated via external service.
8a1 System creates new user account without email and 
    password. #(guard:external_auth) #(goto:9)
